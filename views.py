...
def applyjob(request, id):
    ...
    if request.method == 'POST':
        ...
        job_status = "Rejected" if similarity_percentage < 60 else "Resume Shortlisted"
        ...
        if job_status == "Resume Shortlisted":
            ...
            send_mail(email_subject, email_message, 'cse.takeoff@gmail.com', [user1.email])
...
def send(request, id):
    ...
    if request.method == 'POST':
        ...
        send_mail(email_subject, email_message, 'cse.takeoff@gmail.com', [data.user_id.email])
...
def selectcandidate(request, id, link):
    login = request.session['login']
    data = ApplyJob.objects.get(id=id)
...
def googleLinkForm(request, id):
    login = request.session.get('login')

    if request.method == 'POST':
        link = request.POST.get('googleLink')
        googleMeet = link  # No need for f-string unless adding extra formatting

        data = get_object_or_404(ApplyJob, id=id)
        data.jobstatus = 'Candidate Selected'
        data.save()
...
You can expect to hear from us soon regarding the next steps and details for your interview.
Your Interview link: {googleMeet}
...
Your Team
        ...
        messages.success(request, 'Candidate Selected')
        return redirect('viewtestresponse', id=id)

    return render(request, 'googleLinkForm.html', {'login': login, 'id': id})